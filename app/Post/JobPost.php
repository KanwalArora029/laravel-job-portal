...
class JobPost
{
    ...
    public function store(Request $data): void
    {
        ...
        $this->listing->application_close_date = \Carbon\Carbon::createFromFormat('m/d/Y', $data['closingDate'])->format('Y-m-d');
    ...
    public function updatePost(int $id, Request $data): void
    {
        if ($data->hasFile('feature_image')) {
            $feature_image = $data->file('feature_image')->store('images', 'public');
